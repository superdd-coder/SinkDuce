...
import { findPlayingSegmentIndex } from "@/lib/transcript-playback"
...
function scrollSegmentIntoView(
  ...
  const el = container.querySelector(`[data-seg-idx="${idx}"]`) as HTMLElement | null
  if (!el) return
  if (container.scrollHeight > container.clientHeight + 2) {
    ...
    const offset =
      ...
      containerTop +
      container.scrollTop -
      container.clientHeight * blockRatio +
      el.offsetHeight / 2
...
export function TranscriptTab({
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      scrollSegmentIntoView(container, idx, 0.5)
    })
    // Cite flash only — playing-sentence class keeps focus while audio runs
  ...
  // Highlight + keep the currently playing sentence in view
  const lastPlayingIdxRef = useRef(-1)
  useEffect(() => {
    if (followLive) return
    const idx = findPlayingSegmentIndex(segments, playbackTime)
    setPlayingIdx(idx)
    if (idx < 0) {
    ...
    raf = requestAnimationFrame(() => {
      raf2 = requestAnimationFrame(() => {
        scrollSegmentIntoView(container, idx, 1 / 3)
      })
    })
    return () => {
      cancelAnimationFrame(raf)
