...
export function TimelineView({
  ...
  const mergeCommittedRef = useRef(false)
  const ecr=useCallback(()=>{
    mergeCommittedRef.current = true
    setEcOpen(false)
    setEcTgt(null)
    fetch({silent:true})
  },[fetch])
  const discardMergePrepare = useCallback(async () => {
    const tgt = ecTgt
    setEcOpen(false)
    setEcTgt(null)
    // Opening Merge used to insert an `end` marker before the form; cancel
    // must remove that placeholder or the branch looks closed with no merge node.
    if (tgt?.endNodeId) {
      try {
        await deleteNode(collectionId, tgt.endNodeId)
      } catch {
        /* already gone */
      }
    }
    fetch({silent:true})
  }, [collectionId, ecTgt, fetch])
   ...
   * submit. Creating it here used to mark the branch closed if the user
   * dismissed the dialog without filling the merge node.
   */
  const mergeBranch = useCallback(async (cid: string) => {
    ...
    try {
      ...
      mergeCommittedRef.current = false
  ...
  if (mc) {
    for (const bc of bcs) {
      ...
      if (!bcd) continue
      // Closed only after a real merge node. A leftover `end` placeholder
      // (dialog cancelled) must not hide the branch as completed.
      const done = !!bc.merge_node_id
      ...
      */}
      ...
      <EndChainDialog
        ...
        onOpenChange={(next) => {
          ...
          if (mergeCommittedRef.current) {
            mergeCommittedRef.current = false
            setEcOpen(false)
            return
          }
          void discardMergePrepare()
        }}
        onComplete={ecr}
        groups={groups}
      />
